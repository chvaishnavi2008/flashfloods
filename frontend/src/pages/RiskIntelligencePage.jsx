...
export default function RiskIntelligencePage() {
  ...
  return (
    <div className="space-y-6 pb-12 font-mono">
      {/* Header Banner */}
      <div className="bg-white dark:bg-[#1E293B] border border-[#D7E0E7] dark:border-slate-700 rounded-2xl p-6 shadow-sm dark:shadow-xl space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-blue-50 dark:bg-blue-600/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/40 rounded-xl">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-50 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/40">
                  REAL-TIME SENSOR NETWORK
                </span>
              </div>
              <h2 className="text-2xl font-black text-[#172B3A] dark:text-white mt-0.5">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 dark:text-slate-400">
              Sector: <strong className="text-[#172B3A] dark:text-white">{selectedLocation?.name || 'Chamoli'}</strong> ({selectedLocation?.state})
            </span>
          </div>
        </div>
      ...
      <section className="bg-white dark:bg-[#1E293B] border border-[#D7E0E7] dark:border-slate-700 rounded-2xl p-5 shadow-sm dark:shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2 text-[#172B3A] dark:text-white font-bold text-xs uppercase">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-400 dark:text-slate-500">Automated Scoring Engine</span>
        </div>
      ...
      <section className="bg-white dark:bg-[#1E293B] border border-[#D7E0E7] dark:border-slate-700 rounded-2xl p-5 shadow-sm dark:shadow-xl space-y-4">
        <h3 className="text-xs font-bold uppercase tracking-wider text-[#172B3A] dark:text-white flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
            ...
            <div className="text-lg font-bold text-[#172B3A] dark:text-white">{environmentalData?.rainfall_rate} mm/hr</div>
            <span className="text-[10px] text-slate-500 dark:text-slate-400">Intensity: {environmentalData?.rainfall_intensity}</span>
          </div>

          <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
            <span className="text-[10px] text-slate-500 uppercase block">24h Accumulated</span>
            <div className="text-lg font-bold text-[#172B3A] dark:text-white">{environmentalData?.rainfall_mm} mm</div>
            <span className="text-[10px] text-slate-500 dark:text-slate-400">Forecast: {environmentalData?.rainfall_forecast_trend}</span>
          </div>

          <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
            <span className="text-[10px] text-slate-500 uppercase block">River Capacity</span>
            <div className="text-lg font-bold text-[#172B3A] dark:text-white">{environmentalData?.river_capacity_pct}%</div>
            <span className="text-[10px] text-slate-500 dark:text-slate-400">Trend: {environmentalData?.river_trend}</span>
          </div>

          <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
            <span className="text-[10px] text-slate-500 uppercase block">Soil Saturation</span>
            <div className="text-lg font-bold text-[#172B3A] dark:text-white">{environmentalData?.soil_saturation_pct}%</div>
            <span className="text-[10px] text-slate-500 dark:text-slate-400">Slope: {environmentalData?.slope_deg}°</span>
          </div>
        </div>
      </section>
    </div>
